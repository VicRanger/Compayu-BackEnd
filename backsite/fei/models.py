...
class Thought(models.Model):
    ...
    author = models.ForeignKey('UserProfile', related_name='thought_author', on_delete=models.CASCADE)
    ...
    def __str__(self):
        # return '%s %s %s %s %s %s %s %s %s %s' % (self.title, self.text, self.picture, self.mood_type, self.type_ai, self.type_human, self.create_time, self.modified_time, self.views, self.author)
        return self.title
